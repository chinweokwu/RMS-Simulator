...
class SiteState:
    ...
    def simulate_tick(self) -> dict:
        """Simulates time progression and state transitions (RAISED -> ACKNOWLEDGED -> CLEARED -> HEARTBEAT)."""
        now_iso = datetime.now(timezone.utc).isoformat()
        event_generated = None

        if self.active_alarm is None:
            # Random chance to raise new alarm
